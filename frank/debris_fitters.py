...
class FrankDebrisFit(metaclass=abc.ABCMeta):
    """
    Base class for results of frank debris disc fits.
    ...
    geometry: SourceGeometry object
        ...
        inclination.
    scale_height: array
        Vertical thickness of the disc at the collocation points
    """
    def __init__(self, DHT, geometry, scale_height):
        self._DHT = DHT
        self._geometry = geometry
        self._scale_height = scale_height 

    def _predict(self, q, kz, I, block_size):
        ...
        H2 = 0.5*(2*np.pi*self._scale_height)**2
        ...
        while end < len(q):
            ...
            gauss = np.exp(-np.outer(ki*ki, H2))

            Hmat = self._DHT.coefficients(qi) * gauss
            V.append(np.dot(Hmat, I)) 
    ...
    def predict(self, u, v, I=None, geometry=None, block_size=10**5):
        ...
        if geometry is not None:
            u, v, wz = self._geometry.deproject(u, v, use3D=True)

        q = np.hypot(u, v)
        V = self._predict(q, wz, I, block_size)
    ...
    def predict_deprojected(self, q=None, I=None, geometry=None,
        ...
        """
        V = self._predict(q, q*0, I, block_size)
    ...
    def scale_height(self):
        ...
        return self._scale_height * rad_to_arcsec


class FrankGaussianDebrisFit(FrankDebrisFit):
    """
    ...
    geometry: SourceGeometry object, optional
        ...
        inclination. If not provided, the geometry determined during the
        fit will be used.
    scale_height: array, optional
        Vertical thickness of the disc at the collocation points
    """

    def __init__(self, DHT, fit, geometry=None, scale_height=0):
        FrankDebrisFit.__init__(self, DHT, geometry, scale_height)
        self._fit = fit
...
class FrankLogNormalDebrisFit(FrankDebrisFit):
    """
    Result of a frank fit with a Gaussian brightness model.
...
class FourierBesselDebrisFitter(object):
    """
    ...
    def __init__(self, Rmax, N, geometry, scale_height, nu=0, block_data=True,
        ...
        self._f_scale_height = scale_height 
        self._scale_height = scale_height(self.r) / rad_to_arcsec
    ...
    def _build_matrices(self, u, v, V, weights):
        ...
        # Use only the real part of V. Note that the cos(inc) visibility 
        # correction is not included since it does not apply for optically 
        # thin emission
        V = V.real 
    ...
    def fit(self, u, v, V, weights=1):
        ...
        self._sol = FrankGaussianDebrisFit(self._DHT, fit,
...
class FrankDebrisFitter(FourierBesselDebrisFitter):
    """
    ...
    def __init__(self, Rmax, N, geometry, scale_height, nu=0, block_data=True,
        ...
        if method not in {'Normal', 'LogNormal'}:
            raise ValueError('FrankDebrisFitter supports following mehods:\n\t'
                             '{ "Normal", "LogNormal"}"')
        self._method = method

        super(FrankDebrisFitter, self).__init__(
            Rmax, N, geometry, scale_height, nu, 
            block_data, block_size, verbose)
        ...
        if self._check_qbounds:
            ...
            if self.q[-1] < uv.max():
                raise ValueError(r"ERROR: Last collocation point, {:.3e} \lambda, is at"
                                 ...
                                 " increase N in FrankDebrisFitter (this is"
    ...
    def fit(self, u, v, V, weights=1):
        ...
        """
        if self._verbose:
            logging.info('  Fitting for brightness profile using FrankDebrisFitter')
        ...
        while (not self._filter.check_convergence(pI, pi_old) and
            ...
            if self._verbose and logging.getLogger().isEnabledFor(logging.INFO):
                print('\r    FrankDebrisFitter iteration {}'.format(count),
                      end='', flush=True)
        ...
        if self._method == "Normal":
            ...
            self._sol = FrankLogNormalDebrisFit(self._DHT, fit,
                                          geometry=self._geometry.clone(),
                                          scale_height=self._scale_height)
